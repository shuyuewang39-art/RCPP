...
class GA(object):
    def __init__(self, DNA_size, cross_rate, mutation_rate, pop_size, ):
        ...
        self.pop_all = self.pop

    def selStochasticUniversalSampling(self, individual, k, fitness):
        ...
        individuals_f = np.hstack((individuals,fitness)) # (500, 6)
        s_inds = sorted(individuals_f, key=lambda x:x[6], reverse=True)
        sum_fits = sum(ind[6] for ind in individuals_f)
    ...
    def evolve(self, pop_all, fitness):
        pop = self.selStochasticUniversalSampling(pop_all, self.pop_size, fitness) # 父代,即新的种群
        pop = np.array(pop)[:,0:6]
        # pop = self.select(fitness)
        pop_copy = pop.copy() # 父代
        self.pop_pare = pop.copy() # 父代
        for parent in pop:  # for every parent
            child = self.crossover(parent, pop_copy)
            child = self.mutate(child)
            parent[:] = child
        self.pop_chil = pop  # 子代
        self.pop = pop
...
def ga_order(static_raw, yaw_task=None, curvature=None):
    ...
    N_CITIES = np.shape(area)[0] # 获取任务目标点
    cut = 1000

    # 参数分别是城市数量，基因交叉率，基因突变率，种群数量
    ga = GA(DNA_size=N_CITIES, cross_rate=CROSS_RATE, mutation_rate=MUTATE_RATE, pop_size=POP_SIZE*2)
    ...
    while count < N_GENERATIONS:  # for generation in range(N_GENERATIONS):#N_GENERATIONS
        ...
        ga.evolve(ga.pop, fitness)
        # ga.pop_all = np.vstack((ga.pop_chil, ga.pop_pare)) # 父代 和 子代
