...
from ga_vrp import ga_vrp  # from ga_vrp2 import ga_vrp
...
for i in range(test_num):
    ...
    miles.append(miles0) # [xxx,xxx,xx][dis0]
    task.append(sub_route0) # ['0, 10, 16, 23, 1, 19, 8, 0']


    '''获得遗传算法的结果'''
    tour_indices1,miles1,sub_route1,t_run1 = ga_vrp.run(i)
    ...
    '''获得多机学习算法的结果'''
    # tour_len, order, t_run= test_vrp(x_a,X_val,load,des_pos,demand,l1)
    # tour_indices2 = order.tolist()[0]
    # label_list = tour_indices2
    # routes = get_subroute(order)
    # sub_route2= label(routes)
    # route_f = tran_area(tour_indices2)
    # routes_f = get_subroute(np.array(route_f)[np.newaxis,:])

    # miles2 = calRouteLen(routes_f)
    # dis2 = np.sum(miles2)-all_demand
    # c_d2 = reward(static, tour_indices2) #=tour_len

    # # 存储数据
    # tour_indices.append(tour_indices2)
    # miles.append(miles2) # miles.append([miles1[0]-all_demand])
    ...
    print(c_d0,c_d1,c_d2)  # 不包含内部距离,c_d1
    # 存储数据
    ...
    if i>=0:
        ...
        plt.show()    
        fig.savefig('./task_assign_new'+str(i)+'.png', bbox_inches='tight', dpi=200)
        # plt.close()
